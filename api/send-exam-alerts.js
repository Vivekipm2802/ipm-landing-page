 ...
 * Sends reminder emails at 7d, 3d, 1d, and 0d before deadline.
 */
      ...
      <table width="480" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="padding:32px;">
            <h2 style="margin:0 0 8px;color:#f1f5f9;font-size:22px;font-weight:800;">${examName}</h2>
            ...
            <p style="margin:0 0 12px;color:#94a3b8;font-size:13px;font-weight:600;">WHAT TO DO NOW:</p>
        ...
        <tr>
          <td style="background:#0a0c14;padding:16px 32px;text-align:center;border-top:1px solid #1e2533;">
...
async function sendReminderEmail({ to, examName, lastDate, daysLeft }) {
  await resend.emails.send({
    from: 'IPM Careers <info@ipmcareer.com>',
    to,
...
export default async function handler(req, res) {
  ...
  const results = { sent: [], errors: [], deactivated: 0 };
    ...
    for (const { days, field } of reminders) {
      if (daysLeft === days && !sub[field]) {
        try {
          ...
          console.log(`✓ Sent ${days}d reminder → ${sub.email} (${sub.exam_name})`);
        } catch (err) {
          console.error(`✗ Failed → ${sub.email} / ${sub.exam_name}:`, err.message);
          results.errors.push({ email: sub.email, exam: sub.exam_name, error: err.message });
        }
      }
    }

    // Deactivate subscriptions where deadline has passed by more than 2 days
