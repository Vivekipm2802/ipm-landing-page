    ...
    for (let attempt = 0; attempt <= delays.length; attempt++) {
      ...
      if (r.ok) {
        ...
        if (!cand?.content?.parts) {
          lastErr = new Error(`Gemini ${m} returned no content (finishReason=${cand?.finishReason || 'unknown'}).`);
        ...
        // If caller asked for JSON, validate parseability BEFORE returning.
        // Truncated/malformed JSON is retry-worthy (so the fallback model
        // gets a chance) — not a hard error to surface as a 500.
        if (json) {
          if (finishReason === 'MAX_TOKENS') {
            lastErr = new Error(`Gemini ${m} hit MAX_TOKENS — JSON truncated.`);
            if (attempt < delays.length) { await sleep(delays[attempt]); continue; }
            throw lastErr;
          }
          try { extractJson(text); }
          catch (parseErr) {
            lastErr = new Error(`Gemini ${m} returned unparseable JSON: ${parseErr.message}`);
            if (attempt < delays.length) { await sleep(delays[attempt]); continue; }
            throw lastErr;
          }
        }
...
// Also tolerates Gemini's habit of including bad escape sequences inside
// string values (\* \_ \[ \] from markdown emphasis/links).
export function extractJson(text) {
  ...
  // Pass 1 — direct parse (clean Gemini output)
  try { return JSON.parse(cleaned); } catch (_) {}

  // Pass 2 — repair invalid escape sequences. JSON only allows \" \\ \/ \b \f
  // \n \r \t \uXXXX. Anything else (e.g. \*, \_, \[, \(, \., \-) is a parse
  // error. Strip the offending backslash so the underlying char survives.
  const repaired = cleaned.replace(/\\(?!["\\/bfnrtu])/g, '');
  try { return JSON.parse(repaired); } catch (_) {}

  // Pass 3 — extract balanced braces/brackets and try the full repair on that
  ...
  for (let i = start; i < cleaned.length; i++) {
    if (cleaned[i] === open)  depth++;
      ...
      if (depth === 0) {
        ...
        try { return JSON.parse(slice.replace(/\\(?!["\\/bfnrtu])/g, '')); } catch (_) {}
      }
    }
  }
  throw new Error('Unbalanced or malformed JSON in response.');
}
