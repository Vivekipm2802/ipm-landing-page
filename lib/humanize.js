...
// Em-dash density target: ≤ 1 per 600 chars. If higher, replace half with commas.
function tameEmDashes(s) {
  // First, normalise all variants to em dash for counting
  let txt = s.replace(/—|–|--/g, '—');
  const target = Math.ceil(txt.length / 600);
  const matches = txt.match(/—/g) || [];
  if (matches.length <= target) return txt;

  // Replace every other em dash with comma (or period if at sentence-end-ish)
  let i = 0;
  txt = txt.replace(/—/g, () => {
    i++;
    if (i % 2 === 0) return ',';
