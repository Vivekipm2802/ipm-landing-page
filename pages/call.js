              ...
              <div className="cp-left">
                <div className="cp-results">
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 20, marginBottom: 12 }}>
                    ...
                    <button
                      className="cp-btn"
                      style={{ flex: 1, minWidth: 140, fontSize: '0.85rem', padding: '10px 14px', background: '#833589' }}
                      onClick={() => {
                        const names = data.filter(n => collegesData[n]).map(n => collegesData[n].title);
                        const text = `IPMAT Call Predictor Results\nName: ${formData.fullname || ''}\nCategory: ${(formData.category || '').toUpperCase()}\nScores: SA=${formData.sa || 0}, MCQ=${formData.qa || 0}, VA=${formData.va || 0}\nTotal: ${(parseFloat(formData.sa||0)+parseFloat(formData.qa||0)+parseFloat(formData.va||0))}/360\n\nPredicted Calls (${names.length}):\n${names.map((n,i) => `${i+1}. ${n}`).join('\n')}\n\nGenerated by IPM Careers\nregister.ipmcareer.com/call`;
                        const blob = new Blob([text], { type: 'text/plain' });
                        const url = URL.createObjectURL(blob);
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `IPMAT_Call_Prediction_${(formData.fullname||'Student').replace(/\s+/g,'_')}.txt`;
                        a.click();
                    ...
                    <button
                      className="cp-btn"
                      style={{ flex: 1, minWidth: 140, fontSize: '0.85rem', padding: '10px 14px', background: copied ? '#22c55e' : '#5A2D62' }}
                      onClick={() => {
                        navigator.clipboard.writeText('https://register.ipmcareer.com/call');
                        setCopied(true);
